...
// Calculador de entradas
// por parámetro: { 'Adult': 2, 'Child': 3, 'Senior': 1 }
// devuelve: 49.99 * 2 + 20.99 * 3 + 24.99 * 1
const taquilla = (personas) => {
  /* Object.keys forEach
  let total = 0;
  Object.keys(personas).forEach(tipoPersona => {
    total += zoo.prices[tipoPersona] * personas[tipoPersona];
  });
  return total;
  */

  /* Object.keys reduce
  return Object.keys(personas).reduce((acc, tipoPersona) => {
    return acc + (zoo.prices[tipoPersona] * personas[tipoPersona])
  }, 0);
  */

  // Object.entries
  let total = 0;
  Object.entries(personas).forEach((entrada) => {
    // entrada[0] => clave (tipoPersona)
    // entrada[1] => número de personas de ese tipoPersona
    total += entrada[1] * zoo.prices[entrada[0]];
  });
  return total;
};

const personas = { Adult: 2, Child: 3, Senior: 1 };
console.log(taquilla(personas));
